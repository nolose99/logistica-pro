...
# --- 1. CONFIGURACIÓN (MODO LIMPIO) ---
st.set_page_config(
    page_title="LOGÍSTICA ARAGÓN", 
    page_icon="🚛", 
    layout="wide",
...
@st.cache_resource
def get_geolocator():
    return Nominatim(user_agent="logistica_aragon_master_v2_clean")
    ...
    try:
        ...
        if ' ' in codigo_bruto:
            ...
            if not found:
                ...
                if loc_ciudad:
                    lat_ref, lon_ref, origen_ref = loc_ciudad.latitude, loc_ciudad.longitude, "WEB ☁️"
...
def optimizar_ruta(puntos, nombres):
    # Intentamos obtener matriz de distancias reales (no línea recta) para ordenar mejor
    ...
    while len(ruta_indices) < len(puntos):
        ...
        for i in range(len(puntos)):
            if i not in visitados:
                if matriz: val = matriz[ultimo][i] # Distancia por carretera
                else: val = geodesic(puntos[ultimo], puntos[i]).kilometers # Línea recta (backup)
                if val < mejor_val:
                    mejor_val = val
                    mejor_idx = i
        if mejor_idx != -1:
...
def obtener_datos_ruta(puntos):
    ...
    try:
        ...
        # OJO: Ya no devolvemos duración, solo ruta y km
        return ruta, dist
...
with st.sidebar:
    ...
    st.info("Copia el link del navegador para compartir.")
    st.markdown("---")
    st.markdown("**Versión:** 3.1 (Clean)")
    st.markdown("**Zona:** Aragón Completo")

st.title("🚛 LOGÍSTICA ARAGÓN | OPTIMIZADOR")

col1, col2 = st.columns([1, 1.5], gap="large")

with col1:
    st.markdown("### 1. 📂 Cargar Pedidos")
    archivo = st.file_uploader("Sube el Excel de ruta", type=["xlsx"])
    
    if archivo:
        df = pd.read_excel(archivo)
        st.caption(f"Leídos {len(df)} clientes.")
            
        if st.button("⚡ OPTIMIZAR SECUENCIA", type="primary", use_container_width=True):
            status = st.status("🏗️ Analizando ubicaciones...", expanded=True)
            ...
            for i, fila in df.iterrows():
                ...
                if lat:
                    puntos.append((lat, lon)); nombres.append(cli)
                else: errores.append(cli)
                if "WEB" in str(met) or "DIR" in str(met): time.sleep(0.6)
                progreso.progress((i + 1) / len(df) / 2)
            
            if len(puntos) > 1:
                status.write("🧠 Ordenando paradas por carretera...")
                pts_fin, nom_fin = optimizar_ruta(puntos, nombres)
                
                status.write("🛣️ Dibujando mapa...")
                trazo, km = obtener_datos_ruta(pts_fin)
                progreso.progress(100)
                
                base_google = "https://www.google.com/maps/dir/"
                url_g = base_google + "/".join([f"{lat},{lon}" for lat, lon in pts_fin]) + "/"
                
                st.session_state['res'] = {'p': pts_fin, 'n': nom_fin, 't': trazo, 'k': km, 'l': url_g, 'e': errores}
                status.update(label="✅ ¡Ruta Lista!", state="complete", expanded=False)
            else:
                status.update(label="❌ Faltan datos", state="error")

with col2:
    st.markdown("### 2. 🗺️ Mapa de Ruta")
    
    if 'res' in st.session_state:
        r = st.session_state['res']
        
        # SÓLO MOSTRAMOS LO QUE ES VERDAD: DISTANCIA Y NÚMERO DE PARADAS
        k1, k2 = st.columns(2)
        k1.metric("Distancia Total", f"{r['k']:.1f} km", border=True)
        k2.metric("Paradas", f"{len(r['p'])}", border=True)
        ...
        st.markdown("### 3. 📤 Entregables")
        c_wa, c_ex = st.columns(2)
        
        with c_wa:
            # WHATSAPP SIN TIEMPO, SOLO HECHOS
            msg = f"🚚 *RUTA ASIGNADA* 🚚\n\n📍 {len(r['p'])} Paradas\n📏 {r['k']:.1f} km\n\n🗺️ *Ver Mapa:* {r['l']}"
        ...
        with c_ex:
            ...
            st.download_button("📥 DESCARGAR EXCEL", buffer.getvalue(), "ruta_optimizada.xlsx", "application/vnd.ms-excel", use_container_width=True)
