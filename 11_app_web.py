...
# Sin la clave, no ven nada.
import os

def check_password():
    """Retorna `True` si el usuario tiene la clave correcta."""
    def password_entered():
        if st.session_state["password"] == "ZUERA2026": # <--- CAMBIA ESTO POR LA CLAVE QUE LE DES AL CLIENTE
    ...
    if "password_correct" not in st.session_state:
        ...
        st.text_input("🔑 Introduce tu Licencia de Acceso:", type="password", on_change=password_entered, key="password")
        return False
    elif not st.session_state["password_correct"]:
        # Clave incorrecta
        st.text_input("🔑 Introduce tu Licencia de Acceso:", type="password", on_change=password_entered, key="password")
        st.error("⛔ Licencia no válida o caducada.")
        return False
    else:
        # Clave correcta
        return True
...
if not check_password():
    st.stop() # DETIENE TODA LA APP SI NO HAY CLAVE
